...
$t = array ();
$companies=array();
$companies[]=array("mepid","category","company","income");
  fputcsv($fp, array("mepid","category","company","income"));
...
//  echo ("\n". $mep->Name->full. " ". $m->date . "->". $m->url);
  foreach ($h as $k) {
    ...
    if (!is_array($m->$k)) {
      ...
      continue;
      print_r ($m->$k);die("weirdtoto");
    } else {
        $v = array();
	foreach ($m->$k as $l) {
  fputcsv($fp, array($m->mep_id,$k,str_replace (",","|",$l[0]),$l[1]));
        $v[] = $l[0] . "[".$l[1]."]";
      }
